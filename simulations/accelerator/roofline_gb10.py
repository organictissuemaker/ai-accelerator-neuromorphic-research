...
workloads = {
    ...
    "LLM decode, batch=8":                  (32,     (10, -15)),
    "CNN conv layer\n(high weight reuse)":  (2000,   (10, 10)),
}
...
# Workload points on the roofline
for name, (ai, offset) in workloads.items():
    perf = min(PEAK_FLOPS, BANDWIDTH * ai)
    ax.scatter(ai, perf, s=120, color="#7030A0", zorder=5)
    ax.annotate(name, xy=(ai, perf), xytext=offset,
...
ax.set_ylabel("Attainable Performance (FLOPs/s)", fontsize=11)
ax.set_title("Roofline Model — NVIDIA GB10 (DGX Spark)\n"
             "1 PFLOP FP4 ceiling vs 273 GB/s memory slope", fontsize=12)
